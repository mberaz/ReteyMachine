...
namespace RetryMachine.Framework
{
    public class RetryMachineRunner : IRetryMachineRunner
    {
        ...
        {
            return CreateTasks(new List<RetryCreate> { new RetryCreate(actionName, JsonConvert.SerializeObject(value), 1, runImmediately) },
                taskName, taskId);
        }

        public Task CreateTasks(string actionName, string value, bool runImmediately = false, string taskName = "", string taskId = null)
        {
            return CreateTasks(new List<RetryCreate> { new RetryCreate(actionName, value, 1, runImmediately) }, taskName, taskId);
        }

        public Task CreateTasks(RetryCreate tasks, string taskName = "", string taskId = null)
        {
            return CreateTasks(new List<RetryCreate> { tasks }, taskName, taskId);
        }
